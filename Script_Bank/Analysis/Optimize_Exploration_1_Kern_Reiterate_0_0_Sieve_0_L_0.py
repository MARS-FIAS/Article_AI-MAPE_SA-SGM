...
from Utilities import interpolator, plotful
...
if sup_comp:
    ...
    path = os.path.dirname(os.path.realpath(__file__)) + '/../../Data_Bank/Shallow_Grid/Shallow_Grid_1_Rule_Kern/' + _path
post = '_Posterior.pkl' # Unnecessary!
...
if sup_comp:
    ...
    path = os.path.dirname(os.path.realpath(__file__)) + '/../../Data_Bank/Optimize_Exploration/Optimize_Exploration_1_Kern/'
...
para_set_raw = {
    'N_N': doze[0], 'G_G': doze[1],
    'G_N': doze[2], 'N_G': doze[3]
}
para_set_mode = 0 # {0, 1} # {'No Remap', 'Remap: [A, B] ---->>>> [0, 1]'}
para_set, para_set_true = make_paras(para_set_raw = para_set_raw, para_set_mode = para_set_mode, verbose = False)
print(f'\nPara Set True!\n{para_set_true}')
